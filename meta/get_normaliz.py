...
class FrozenBatchNorm2d(nn.Layer):
    ...
    def forward(self, x):
        if not x.stop_gradient:
            ...
            scale = self.weight_attr * (self.running_var + self.eps).rsqrt()
            bias = self.bias_attr - self.running_mean * scale
            scale = scale.reshape([1, -1, 1, 1])
            bias = bias.reshape([1, -1, 1, 1])
            out_dtype = x.dtype  # may be half
            return x * scale.astype(out_dtype) + bias.astype(out_dtype)
        else:
