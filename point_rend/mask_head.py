...
from utils.pointrcfg import configurable
...
def calculate_uncertainty(logits, classes):
    ...
    if logits.shape[1] == 1:
        ...
        gt_class_logits = logits[
            paddle.arange(logits.shape[0]), classes
        ].unsqueeze(1)
    return -(paddle.abs(gt_class_logits))


class ConvFCHead(nn.Layer):
    """
    ...
    def forward(self, x):
        ...
        for layer in self.conv_layers:
            x = layer(x)
        x = paddle.flatten(x, start_dim=1)
        for layer in self.fcs:
            x = F.relu(layer(x))
...
class ImplicitPointRendMaskHead(PointRendMaskHead):
    ...
    def _uniform_sample_train_points(self, instances):
        ...
        cat_boxes = Boxes.cat(proposal_boxes)
        # uniform sample
        point_coords = paddle.rand([len(cat_boxes), self.mask_point_train_num_points, 2])
        # sample point_labels
        point_coords_wrt_image = get_point_coords_wrt_image(cat_boxes.tensor, point_coords)
        point_labels = sample_point_labels(instances, point_coords_wrt_image)
        return point_coords, point_labels
