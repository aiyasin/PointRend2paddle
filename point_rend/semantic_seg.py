...
def calculate_uncertainty(sem_seg_logits):
    ...
    """
    top2_scores = paddle.topk(sem_seg_logits, k=2, axis=1)[0]
    return (top2_scores[:, 1] - top2_scores[:, 0]).unsqueeze(1)


@SEM_SEG_HEADS_REGISTRY.register()
class PointRendSemSegHead(nn.Layer):
    """
    ...
    def forward(self, features, targets=None):
        ...
        if self.training:
            ...
            fine_grained_features = paddle.concat(
                [
            ...
            point_targets = (
                point_sample(
                    targets.unsqueeze(1).astype('float32'),
                    point_coords,
                    mode="nearest",
                    align_corners=False,
                )
